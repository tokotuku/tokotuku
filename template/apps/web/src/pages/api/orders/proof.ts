...
export const POST: APIRoute = async ({ request, redirect }) => {
  ...
  if (!session) return redirect("/login?next=/checkout");
  ...
  try {
    ...
    await env.PRODUCT_IMAGES.put(`payment-proofs/${proofKey}`, proof.stream(), {
      httpMetadata: { contentType: proof.type },
    });
    await attachPaymentProof(env.DB, orderNumber, session.user.id, proofKey);
  } catch (caught) {
