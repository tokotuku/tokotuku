...
`create-karsa` is now an interactive wizard that finishes setup itself,
...
(brand name, locale, optional currency, time zone), and whether to `git init`. It then
runs install, `cf-typegen`, `karsa db sync`, and `wrangler d1 migrations
...
Generating `.dev.vars` also closes a real gap. `@karsa/auth` passes
...
only, print the manual steps), `--seed` (also run `karsa db seed`), and
`--registry <url>` (write a scoped `.npmrc`, for resolving `@karsa/*` from a
private registry).
...
`@karsa/auth` now requires `better-auth >=1.6.23 <1.7.0`. 1.7.0 adds an
