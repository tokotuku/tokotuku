...
| Karsa module/data architecture | 24 | 25 |
...
Scoring: **95/100** — responsive hierarchy, local imagery, navigation, contrast, overflow, and browser console were inspected at both required viewports with no console errors or warnings.
...
- astro.config.mjs configures only auth({ registration: "closed" }) and jarene(); package.json contains no catalog, orders, or booking dependency.
- src/pages/index.astro is the sole app-owned content page, uses SiteHeader/SiteFooter/Card with explicit site links and search disabled, and contains no copied commerce surface.
- src/middleware.ts registers only @karsa/auth/register; src/pages/robots.txt.ts conditionally mentions commerce paths only when optional registry modules are installed.
...
- Desktop and mobile captures confirm that the 720px responsive rule converts the hero, intro, operations, and team grids to a readable single-column mobile layout.

## Strengths

- Company-profile scope is correctly preserved: auth and jarene are the only installed modules, commerce dependencies are absent, and the header receives only explicit site links with search disabled.
