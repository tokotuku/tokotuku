...
- astro.config.mjs:15 sets name to Racik Rasa; astro.config.mjs:25 declares public auth, jarene(), catalog({ presentation: "products" }), and orders({ presentation: "orders" }).
- package.json:22, package.json:24, package.json:28, and package.json:29 contain the corresponding Karsa dependencies; ../../bun.lock records product-install with the same dependency set.
- karsa.migrations.json:2 records nextSequence 6 and auth:1, catalog:3, orders:2; migrations/0000_auth_init.sql through migrations/0005_orders_payments-bank-transfer.sql are contiguous and hash-identical to their package migration sources.
- src/pages/index.astro:2-5 delegates the storefront to @karsa/core; src/pages/robots.txt.ts:3-20 uses the virtual registry; no app-owned commerce routes exist.
