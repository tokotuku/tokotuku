...
- astro.config.mjs sets brand.name to "Arunika Energi" and configures modules: [auth({ registration: "closed" }), jarene()]; package.json has no catalog, orders, or booking dependency.
- migrations/0000_auth_init.sql is byte-identical to ../../packages/auth/migrations/0001_init.sql, and karsa.migrations.json records {"auth":1}.
...
- src/pages/index.astro renders the neutral @karsa/core SiteHome component, and the source scan found no @karsa/catalog, @karsa/orders, or @karsa/booking imports.
- No screenshots were supplied; no visual claims were inferred.

## Strengths

- Brand is exactly "Arunika Energi" and the configuration contains only closed auth plus scaffold-baseline jarene().
- The only migration exactly matches @karsa/auth's module migration; the migration lock records auth at sequence 1.
...
- App-owned routes are limited to the packaged SiteHome entry point and a module-aware robots route.
