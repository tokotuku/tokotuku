...
export interface ListItemsOptions {
  ...
  presentation?: "products" | "services";
}
...
const itemFrom = "FROM catalog_items ci LEFT JOIN inventory_item_stock s ON s.item_id = ci.id";
...
}: Pick<ListItemsOptions, "activeOnly" | "category" | "search" | "presentation">) {
  const conditions: string[] = [
    presentation === "products"
      ? "ci.fulfillment_type = 'physical' AND ci.price_cents IS NOT NULL"
      : "ci.fulfillment_type IN ('scheduled', 'project')",
  ];
...
export async function listItems(
  ...
  const { where, bindings } = buildItemFilter({ activeOnly, category, search, presentation });
  ...
  const query = `SELECT ${itemColumns} ${itemFrom}${where}${cursorWhere} ORDER BY ci.id ${direction} ${pagination}`;
  ...
  const pageRows = results.slice(0, size).map(toCatalogItem);
  if (before) pageRows.reverse();
  const first = pageRows[0];
  const last = pageRows.at(-1);
  const makeCursor = (product: CatalogItem | undefined): string | null =>
...
export async function countItems(
  ...
  const { where, bindings } = buildItemFilter({ activeOnly, category, search, presentation });
  const row = await db
    .prepare(`SELECT COUNT(*) AS count ${itemFrom}${where}`)
    .bind(...bindings)
    .first<{ count: number }>();
  return row?.count ?? 0;
}
...
export async function getCatalogDashboardSummary(
  db: D1Database,
...
export async function listCategories(
  ...
  const { results } = await db
    .prepare(
      `SELECT DISTINCT category FROM catalog_items WHERE is_active = 1 AND ${presentation === "products" ? "fulfillment_type = 'physical' AND price_cents IS NOT NULL" : "fulfillment_type IN ('scheduled', 'project')"} ORDER BY category ASC`,
    )
    .all<{ category: string }>();
...
export async function findItemById(
  ...
  id: number,
  {
    ...
    presentation = "products",
  }: { includeInactive?: boolean; presentation?: "products" | "services" } = {},
): Promise<CatalogItem | null> {
  const row = await db
    .prepare(
      `SELECT ${itemColumns} ${itemFrom} WHERE ci.id = ? AND ${presentation === "products" ? "ci.fulfillment_type = 'physical' AND ci.price_cents IS NOT NULL" : "ci.fulfillment_type IN ('scheduled', 'project')"}${includeInactive ? "" : " AND ci.is_active = 1"}`,
    )
    .bind(id)
...
export async function findItemsByIds(db: D1Database, ids: number[]): Promise<CatalogItem[]> {
  if (!ids.length) return [];
  const placeholders = ids.map(() => "?").join(", ");
  const { results } = await db
    .prepare(
      `SELECT ${itemColumns} ${itemFrom} WHERE ci.id IN (${placeholders}) AND ci.is_active = 1 AND ci.fulfillment_type = 'physical' AND ci.price_cents IS NOT NULL`,
    )
    .bind(...ids)
    .all<CatalogItemRow>();
...
export async function createItem(db: D1Database, input: CatalogItemInput): Promise<number> {
  const row = await db
    .prepare(
    ...
    .first<{ id: number }>();
  if (!row) throw new Error("CatalogItem could not be created");
...
export async function updateItem(db: D1Database, id: number, input: CatalogItemInput) {
  const exists = await db.prepare("SELECT 1 FROM catalog_items WHERE id = ?").bind(id).first();
  if (!exists) throw new Error("CatalogItem not found");
  ...
  // see the matching branch in createItem.
  if (input.fulfillmentType === "physical") {
...
export async function archiveItem(db: D1Database, id: number) {
  await db
