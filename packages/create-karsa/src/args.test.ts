...
describe("parseArgs", () => {
  it("accepts a bare project name with no flags", () => {
    ...
    expect(result).toEqual({
      ok: true,
      args: { projectName: "my-store", flags: new Set(), registry: undefined, preset: undefined },
    });
  });
  ...
  // `create-karsa app1 app2` scaffolded only app1.
  ...
  it("accepts all four presets and rejects unknown ones", () => {
    for (const preset of ["company", "product", "service", "publication"]) {
      const result = parseArgs(["site", "--preset", preset]);
      expect(result.ok && result.args.preset).toBe(preset);
    }
    expect(parseArgs(["site", "--preset", "shop"]).ok).toBe(false);
  });
});
